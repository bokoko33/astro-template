export default class EMath {
  ...
  static lerp2d = (vec1, vec2, alpha) => {
    const x = this.lerp(vec1.x, vec2.x, alpha);
    const y = this.lerp(vec1.y, vec2.y, alpha);

    return { x, y };
  };

  static lerp3d = (vec1, vec2, alpha) => {
    const x = this.lerp(vec1.x, vec2.x, alpha);
    const y = this.lerp(vec1.y, vec2.y, alpha);
    const z = this.lerp(vec1.z, vec2.z, alpha);
